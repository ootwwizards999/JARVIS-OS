...
// KnowledgeGraph/NeuralDetail "last run" labels — assumes a run it reads is
// already complete. Keeping the shared `AgentRun` type's finishedAt as a
// plain string preserves that. The scheduler's write path (db.agentRuns.insert)
// uses its own wider parameter type that allows the null claim value; reads
// go through AgentRunSchema.parse, the actual runtime source of truth.
